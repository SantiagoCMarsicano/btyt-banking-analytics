...
Banks Data Dictionary --- V4.0.0

Primary structural file: banks.csv
Domestic temporal state: bank_market_weights.csv
Macro/shock state: bank_macro_environment.csv
Related temporal financial file: bank_financials.csv
Project: Banco de Treinta y Tres (BTYT) --- Banking Analytics
Version: 4.0.0
Status: Final design contract before Python implementation
Baseline: V3.0.7
...
any modeled relationship between BTYT and a real institution exists
in reality.
...
(t \in {=tex}{2021,\ldots{=tex},2026}).

Domestic competitive position is therefore a stochastic time series.

The annual state is persistent:

[ State_t \rightarrow{=tex} State_{t+1} ]

rather than:

[ IndependentDraw_t \rightarrow{=tex} IndependentDraw_{t+1} ]

The objective is path dependence.

4. Banking network

BTYT BANKING NETWORK
├── BTYT
│   └── BTYT
│
├── DOMESTIC COMPETITIVE SYSTEM
│   ├── BROU
│   ├── Santander Uruguay
│   ├── Itaú Uruguay
│   ├── BBVA Uruguay
...
│   ├── Citibank Uruguay
│   ├── BHU
│   ├── HSBC Uruguay / BTG Pactual Uruguay
│   ├── Banco Nación
...
B000        BTYT
B001-B099   DOMESTIC / URUGUAYAN SYSTEM
B101-B199   FOREIGN
...
B010    BTG Pactual Uruguay
B011    Scotiabank Uruguay
B101    JPMorgan Chase
B102    Citibank International
B103    Bank of America
B104    Deutsche Bank
B105    BNP Paribas
B106    UBS
B107    HSBC International
...
2021 `B010`      HSBC Uruguay
2022 `B010`      HSBC Uruguay
2023 `B010`      HSBC Uruguay
2024 `B010`      HSBC Uruguay
2025 `B010`      HSBC Uruguay
2026 `B010`      BTG Pactual Uruguay

banks.csv remains a current-state dimension and therefore uses
BTG Pactual Uruguay.
...
bank_id                   string                              No Stable synthetic bank
identifier.

bank_name                 string                              No Current-state display
name.

bank_scope                categorical                         No BTYT, DOMESTIC,
or FOREIGN.

bank_type                 categorical                         No Broad institutional
role.

operating_country         string                              No Operating
jurisdiction used by
the simulation.

bank_profile              categorical                         No Structural behavioral
profile.

usd_affinity              decimal                             No Structural USD
anchor.

business_affinity         decimal                             No Structural
business-customer
anchor.

large_transfer_affinity   decimal                             No Structural
high-value-transfer
anchor.

The V3 fields market_weight_low and market_weight_high are no longer
the primary representation of domestic competitive position.
...
BTYT                      INTERNAL_GENERALIST
BROU                      GENERALIST_PUBLIC_RETAIL
BHU                       MORTGAGE_SPECIALIST
Bandes                    RETAIL_NICHE
BBVA Uruguay              GENERALIST_RETAIL_BUSINESS
Banco Nación              REGIONAL_CROSS_BORDER
Itaú Uruguay              GENERALIST_PREMIUM_BUSINESS
Santander Uruguay         GENERALIST_RETAIL_BUSINESS
Banque Heritage Uruguay   PRIVATE_WEALTH
Citibank Uruguay          CORPORATE_USD
B010                      CORPORATE_WEALTH_USD
Scotiabank Uruguay        GENERALIST_RETAIL_BUSINESS
JPMorgan Chase            CORPORATE_TREASURY_USD
Citibank International    CORPORATE_TREASURY_USD
Bank of America           CORPORATE_USD
Deutsche Bank             CORPORATE_INTERNATIONAL
BNP Paribas               CORPORATE_INTERNATIONAL
UBS                       PRIVATE_WEALTH_INTERNATIONAL
HSBC International        CROSS_BORDER_CORPORATE
...
[ \sum{=tex}{i \in {=tex}UruguaySystem} w{i,t}=1 ]
...
BROU                           24.0%         30.0%       36.0%
Santander Uruguay              10.0%         15.0%       21.0%
Itaú Uruguay                   10.0%         14.0%       21.0%
BBVA Uruguay                    6.0%         10.0%       15.0%
Scotiabank Uruguay              6.0%          9.0%       14.0%
BTYT                            2.5%          5.0%        8.0%
Citibank Uruguay                1.5%          4.0%        7.0%
BHU                             1.5%          3.0%        6.0%
B010                            1.0%          3.0%        6.0%
Banco Nación                    1.0%          2.5%        5.0%
Banque Heritage Uruguay         0.5%          2.0%        4.0%
Bandes                          0.5%          1.5%        3.5%
...
[ z_{i,t} = \mu{=tex}_i +
\phi{=tex}i(z{i,t-1}-\mu{=tex}_i) + \beta{=tex}i M_t +
\gamma{=tex}i S_t + \delta{=tex}{i,t} + \epsilon{=tex}{i,t} ]

where:

(\mu{=tex}_i) = bank-specific long-run competitive anchor;

(\phi{=tex}_i) = persistence;

(M_t) = macro environment;

(S_t) = systemic banking shock;

(\beta{=tex}_i) = macro sensitivity;

(\gamma{=tex}_i) = systemic-shock sensitivity;

(\delta{=tex}_{i,t}) = bank-specific shock;

(\epsilon{=tex}_{i,t}) = ordinary idiosyncratic innovation.

Annual market weights are:

[ w_{i,t} = \frac{\exp(z_{i,t})}{=tex} {\sum{=tex}j
\exp{=tex}(z{j,t})} ]
...
[ \epsilon{=tex}_{i,t} \sim{=tex} N(0,\sigma{=tex}_i) ]
...
V4 adds bank_macro_environment.csv.

The macro environment is shared by all banks but affects them
differently through bank-specific sensitivities.

The DGP may represent broad synthetic dimensions such as:
...
These variables are synthetic latent drivers.

They are not intended to reproduce the historical Uruguayan
...
[ Effect_{i,t}^{systemic} = \gamma{=tex}_i S_t ]
...
ordinary annual innovation      common
material bank-specific shock    occasional
large bank-specific shock       rare
systemic stress episode         occasional / rare
...
bank_id                    string                              No Stable bank
identifier.

bank_name                  string                              No Historically
appropriate display
name.

year                       integer                             No 2021--2026.

market_weight              decimal                             No Realized synthetic
domestic competitive
weight.

latent_competitive_state   decimal                             No Latent state before
softmax.

long_run_anchor            decimal                             No Bank-specific
latent/competitive
anchor.

persistence                decimal                             No Bank-specific AR
persistence.

macro_effect               decimal                             No Contribution from
macro conditions.

systemic_shock_effect      decimal                             No Contribution from
systemic shock.

bank_shock_effect          decimal                             No Bank-specific
material shock
contribution.

idiosyncratic_innovation   decimal                             No Ordinary annual
innovation.

is_bank_shock              boolean                             No Material
bank-specific shock
flag.

is_systemic_shock          boolean                             No Systemic-shock-year
flag.
...
21. bank_macro_environment.csv

21.1 Purpose

Stores the shared annual environment driving the banking system.
...
macro_growth_factor       decimal                             No Synthetic economic-cycle
factor.

credit_cycle_factor       decimal                             No Synthetic credit-demand /
credit-cycle state.

usd_pressure_factor       decimal                             No Synthetic FX/USD pressure
state.

financial_stress_factor   decimal                             No Synthetic system stress.

digitalization_factor     decimal                             No Synthetic
competitive/digital
pressure.

cross_border_factor       decimal                             No Synthetic
international/cross-border
state.

systemic_shock            decimal                             No Signed systemic banking
shock.

systemic_shock_flag       boolean                             No Whether a material systemic
shock occurred.

world_seed                integer                             No Synthetic world seed.

The precise macro-factor implementation belongs to the generator, but
the distinction between common environment and bank-specific response is
frozen.
...
[ (w_1,\ldots{=tex},w_7) \sim{=tex}
Dirichlet(\alpha{=tex},\ldots{=tex},\alpha{=tex}) ]

with:

[ \alpha{=tex}_{world} \sim{=tex} Uniform(2.5,6.0) ]
...
[ score_{i,t} = market_weight_{i,t} \times{=tex} currency_fit_i
\times{=tex} customer_fit_i \times{=tex} amount_fit_i \times{=tex}
behavioral_fit_i ]

and:

[ P(bank=i \mid {=tex}transfer,t) = \frac{score_{i,t}}{=tex}
{\sum{=tex}j score{j,t}} ]
...
[ a_{i,k}^{world} = clip(a_{i,k}^{base}+\eta{=tex}_{i,k},0,1) ]
...
temporal identity changes from HSBC Uruguay to BTG Pactual Uruguay
while bank_id remains stable.
...
[ SalaryReceipt_t \rightarrow{=tex} P(BHUPayment_{t+1+7})
\uparrow{=tex} ]
...
DOMESTIC_EXTERNAL                 External transfer involving another
domestic bank.
...
OTHER_BANK + domestic institution
→ counterparty_bank_id = B001-B011
→ transfer_scope = DOMESTIC_EXTERNAL

OTHER_BANK + foreign institution
→ counterparty_bank_id = B101-B107
...
realized_large_transfer_affinity   World-specific large-transfer
affinity.

foreign_selection_weight           Foreign counterparty weight when
applicable.
...
revenue              decimal                             No Synthetic annual
operating
revenue.

operating_costs      decimal                             No Synthetic
operating costs.

net_income           decimal                             No Synthetic annual
net income.

total_assets         decimal                             No Synthetic
year-end assets.

total_deposits       decimal                             No Synthetic
year-end
deposits.

total_loans          decimal                             No Synthetic gross
loan portfolio.

equity               decimal                             No Synthetic
year-end equity.
...
[ AssetGrowth_{i,t} = g_i + \lambda{=tex}i
\Delta {=tex}MarketWeight{i,t} + \theta{=tex}i Macro_t +
\kappa{=tex}i Shock{i,t} + u{i,t} ]
...
residual world-level financial parameters for components not
explicitly modeled.
...
[ CostToIncome = \frac{operating\_costs}{revenue}{=tex} ]

ROA

[ ROA = \frac{net\_income}{average\ total\ assets}{=tex} ]

ROE

[ ROE = \frac{net\_income}{average\ equity}{=tex} ]

Loan-to-deposit

[ LTD = \frac{total\_loans}{total\_deposits}{=tex} ]

Net margin

[ NetMargin = \frac{net\_income}{revenue}{=tex} ]
...
2021 domestic initialization
domestic temporal dynamics
macro environment
systemic shocks
...
B010 + 2021-2025 → HSBC Uruguay
B010 + 2026      → BTG Pactual Uruguay
...
BROU begins in the structural top tier and the initialization
remains plausible;
...
B010 displays as HSBC Uruguay in 2021--2025 and
BTG Pactual Uruguay in 2026.
...
the major private-bank tier is led structurally by Santander and
Itaú centers, but later ranking is not scripted.
...
bank_macro_environment.csv stores shared annual macro/shock state.

bank_world_parameters.csv stores world-level parameters that are
not annual domestic market states.

domestic financial trajectories are partially linked to competitive
dynamics and macro conditions.

BTYT financials should increasingly connect to its own generated
banking activity.
...
scripts/generate_banks_v4_0_0.py

The intended generation order is:

1. build structural banks dimension
2. generate world-level structural parameters
3. generate 2021 domestic competitive state
4. generate 2022-2026 macro environment
5. generate systemic and bank-specific shocks
6. evolve domestic latent competitive states
7. convert latent states to annual market weights
8. generate foreign counterparty weights
9. generate realized behavioral affinities
10. generate financial trajectories linked to the V4 state
11. validate all tables
12. save outputs
13. run cross-world audit
...
Next step: implement generate_banks_v4_0_0.py from this frozen
V4.0.0 contract and run the domestic cross-world audit before accepting
