...
Canonical sources:
    docs/data_dictionary/loans_data_dictionary.md
    docs/data_dictionary/loan_monthly_snapshot_data_dictionary.md
...
from scripts.core.paths import GENERATED_CORE_DIR, INTERIM_CREDIT_DIR, INTERIM_WORLD_DIR
...
RNG_NAMESPACE = "loans"
RNG_STREAM_MAIN = 0
RNG_STREAM_HISTORY = 1
RNG_STREAM_BRIDGE_BASE = 700_000
...
OUTPUT_PARQUET_PATH = GENERATED_CORE_DIR / "loans.parquet"
OUTPUT_CSV_PATH = GENERATED_CORE_DIR / "loans.csv"
...
def load_inputs():
    customer_source = first_existing([CUSTOMERS_PARQUET_PATH, CUSTOMERS_CSV_PATH])
    account_source = first_existing([ACCOUNTS_PARQUET_PATH, ACCOUNTS_CSV_PATH])

    customers = (
        pd.read_parquet(customer_source)
        if customer_source.suffix.lower() == ".parquet"
        else pd.read_csv(customer_source)
    )
    accounts = (
        pd.read_parquet(account_source)
        if account_source.suffix.lower() == ".parquet"
        else pd.read_csv(account_source)
    )

    required_paths = [BRANCHES_PATH, PRODUCTS_PATH, BRANCH_STATE_PATH]
    ...
    customers["primary_branch_id"] = customers["primary_branch_id"].astype(str).str.replace(r"\.0$", "", regex=True).str.zfill(3)
    accounts["account_id"] = accounts["account_id"].astype(str)
    accounts["customer_id"] = accounts["customer_id"].astype(str)
    accounts["product_id"] = accounts["product_id"].astype(str)
    accounts["branch_id"] = accounts["branch_id"].astype(str).str.replace(r"\.0$", "", regex=True).str.zfill(3)
    branches["branch_id"] = branches["branch_id"].astype(str).str.replace(r"\.0$", "", regex=True).str.zfill(3)
    branch_state["branch_id"] = branch_state["branch_id"].astype(str).str.replace(r"\.0$", "", regex=True).str.zfill(3)

    customers = customers.sort_values("customer_id").reset_index(drop=True)
    accounts = accounts[accounts["customer_id"].isin(set(customers["customer_id"]))].copy()
...
class LoanGenerator:
    def __init__(self, customers, accounts, branches, products, branch_state):
        ...
        self.rng = make_rng(WORLD.seed, RNG_NAMESPACE, RNG_STREAM_MAIN)
        self.history_rng = make_rng(WORLD.seed, RNG_NAMESPACE, RNG_STREAM_HISTORY)
        self.rows = []
        self.bridge_rows = []
    ...
    def _build_bridge_row(
        ...
        """
        loan_number = int(loan_id[1:])
        lifecycle_seed = RNG_STREAM_BRIDGE_BASE + loan_number
        bridge_rng = make_rng(WORLD.seed, RNG_NAMESPACE, lifecycle_seed)
...
def main():
    ...
    if WRITE_COMPATIBILITY_CSV:
        loans.to_csv(OUTPUT_CSV_PATH, index=False)
