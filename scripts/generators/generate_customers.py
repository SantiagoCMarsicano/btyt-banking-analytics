...
END_YEAR = WORLD.end_date.year
BANK_START = 1969
MIN_AGE = 18

RNG_NAMESPACE = "customers"
RNG_STREAM_GLOBAL = 0
RNG_STREAM_ENTITY_BASE = 10_000

def make_global_rng() -> np.random.Generator:
    """Create the deterministic customers-level global RNG."""
    return make_rng(
        world_seed=WORLD.seed,
        namespace=RNG_NAMESPACE,
        stream=RNG_STREAM_GLOBAL,
    )


_active_rng = make_global_rng()


def set_active_rng(rng: np.random.Generator) -> None:
    """Select the RNG used by stochastic helper functions."""
    global _active_rng
    _active_rng = rng


def active_rng() -> np.random.Generator:
    """Return the currently selected RNG."""
    return _active_rng


def make_customer_rng(customer_index: int) -> np.random.Generator:
    """Create a deterministic RNG dedicated to one customer entity."""
    if customer_index < 0:
        raise ValueError(
            "customer_index must be greater than or equal to zero."
        )

    return make_rng(
        world_seed=WORLD.seed,
        namespace=RNG_NAMESPACE,
        stream=RNG_STREAM_ENTITY_BASE + customer_index,
    )

BRANCHES = GENERATED_CORE_DIR / "branches.csv"
PARQUET_OUT = GENERATED_CORE_DIR / "customers.parquet"
CSV_OUT = GENERATED_CORE_DIR / "customers.csv"
...
def choice(v,w): return active_rng().choice(list(v),p=norm(w))
def mult(c=1,d=.05): return max(float(active_rng().normal(c,d)),.05)
def internal(x): return None if pd.isna(x) else x

def lognormal(median,sigma,maximum):
    for _ in range(100):
        x=float(active_rng().lognormal(math.log(max(median,1)),sigma))
...
def person_name(nat,gender,birth):
    ...
    if gender=='OTHER_OR_UNSPECIFIED' and active_rng().random()<.70:
        first=str(active_rng().choice(p['N']))
        cohort='NEUTRAL'
    else:
...
def nationality():
    if active_rng().random()<active_rng().uniform(.85,.94):return 'URUGUAY'
    return choice(['ARGENTINA','BRAZIL','VENEZUELA','CUBA','OTHER'],[active_rng().uniform(.18,.28),active_rng().uniform(.22,.34),active_rng().uniform(.16,.27),active_rng().uniform(.05,.12),active_rng().uniform(.08,.16)])

def gender():
    o=active_rng().uniform(.005,.025); f=active_rng().uniform(.47,.53)
    return choice(['FEMALE','MALE','OTHER_OR_UNSPECIFIED'],[(1-o)*f,(1-o)*(1-f),o])

def residence_country(ctype,nat):
    p=active_rng().uniform(.005,.025) if ctype=='BUSINESS' else active_rng().uniform(.015,.045)
    if nat in {'ARGENTINA','BRAZIL'}:p*=1.6
    return 'URUGUAY' if active_rng().random()>=min(p,.15) else choice(['ARGENTINA','BRAZIL','SPAIN','USA','OTHER'],[.30,.28,.15,.12,.15])
...
def uy_residence(nat):
    ...
    for d in deps:
        ...
        ws.append(w*active_rng().lognormal(0,.08))
    d=str(choice(deps,ws)); l=str(choice(LOC[d].keys(),LOC[d].values())); return d,l

# ---------------- economics ----------------
def birth_year():
    b=choice(['18_24','25_34','35_49','50_64','65_79','80_95'],[.08,.21,.31,.24,.13,.03]); lo,hi={'18_24':(18,24),'25_34':(25,34),'35_49':(35,49),'50_64':(50,64),'65_79':(65,79),'80_95':(80,95)}[b]; return END_YEAR-int(active_rng().integers(lo,hi+1))
def foundation_year():
    b=choice(['0_5','6_10','11_20','21_35','36_55','56_80','81_110'],[.22,.19,.24,.18,.11,.05,.01]); lo,hi={'0_5':(0,5),'6_10':(6,10),'11_20':(11,20),'21_35':(21,35),'36_55':(36,55),'56_80':(56,80),'81_110':(81,110)}[b]; return max(1900,END_YEAR-int(active_rng().integers(lo,hi+1)))
def employment(age):
...
def company_size(s,fy,d):
    w={'MICRO':active_rng().uniform(55,70),'SMALL':active_rng().uniform(20,32),'MEDIUM':active_rng().uniform(5,12),'LARGE':active_rng().uniform(.5,3)}
...
def company_name(s,sp,size,d,l):
    pre=str(active_rng().choice(PREFIX[s])); family=NAMES['URUGUAY']['L']+NAMES['BRAZIL']['L']
    ...
    if active_rng().random() < (.52 if size in {'MICRO','SMALL'} else .25): name=f'{pre} {active_rng().choice(family)}'
    else:name=f"{pre} {l if l and not str(l).startswith('Other ') and active_rng().random()<.25 else active_rng().choice(['del Este','del Plata','Oriental','del Olimar','del Norte','del Sur','Uruguay'])}"
    if size=='LARGE' and active_rng().random()<.55:name+=' S.A.'
    elif size in {'SMALL','MEDIUM'} and active_rng().random()<.2:name+=' Ltda.'
    return name
...
def revenue(size,s,fy):
    ...
    med*=mult(.78,.15) if age<=3 else mult(.93,.12) if age<=10 else mult(1.1,.15) if age>=30 else 1;med*=.7+float(np.clip(active_rng().normal(.5,.15),0,1))*.6;return lognormal(med,sg,100_000_000_000)

# ---------------- documents/history/branches ----------------
def doc_type(ct,nat,country):
    if ct=='BUSINESS':return 'RUT'
    if nat=='URUGUAY':return 'CI'
    if country=='URUGUAY' and active_rng().random()<.28:return 'CI'
...
def doc_id(t,birth,used):
    if t=='CI':
        age=END_YEAR-int(birth);lo,hi=(800000,3400000) if age>=80 else (1000000,4200000) if age>=65 else (1500000,4800000) if age>=50 else (2500000,5500000) if age>=35 else (3500000,5900000) if age>=25 else (4500000,6000000);return unique_doc(lambda:f'CI{int(active_rng().integers(lo,hi+1)):07d}',used)
    letters=list('ABCDEFGHIJKLMNOPQRSTUVWXYZ')
    if t=='PASSPORT':return unique_doc(lambda:'P'+''.join(active_rng().choice(letters,2))+str(int(active_rng().integers(1_000_000,10_000_000))),used)
    if t=='FOREIGN_ID':return unique_doc(lambda:'F'+''.join(active_rng().choice(letters,2))+str(int(active_rng().integers(10_000_000,100_000_000))),used)
    return unique_doc(lambda:f'RUT{int(active_rng().integers(10_000_000_000,100_000_000_000))}',used)
def shocks():
    d={};prev=0
    for y in range(BANK_START,END_YEAR+1):prev=.72*prev+active_rng().normal(0,.08);d[y]=math.exp(prev)
    return d
...
def historical_res(country,d,l,reg):
    ...
    if active_rng().random()>min(.04+(END_YEAR-reg)*.006,.28):return 'URUGUAY',d,l
...
def _choose_branch_inside(pool,reg):
    ...
    for _,r in pool.iterrows():
        ...
        w*=active_rng().lognormal(0,.18)
        ws.append(w)

    idx=active_rng().choice(pool.index.to_numpy(),p=norm(ws))
...
def primary_branch(reg,country,d,l,b):
    ...
    if country!='URUGUAY':
        ...
        selected_pool=pools[int(active_rng().choice(
            np.arange(len(pools)),
            p=norm(weights)
        ))]
...
def lifecycle(reg,bid,b):
    p=(1-(1-.035)**max(END_YEAR-reg,1))*active_rng().uniform(.75,1.25);br=b[b.branch_id==bid].iloc[0];bc=br.closing_year;p*=1.18 if pd.notna(bc) else 1;p=min(float(p),.55)
    if active_rng().random()>=p:return 'ACTIVE',pd.NA
...
def _build_customer_chunk(
    ...
    """Generate one deterministic customer chunk."""
    rows = []
    name_cohorts = []

    for i in range(chunk_start, chunk_end):
        ct = types[i]
        set_active_rng(make_customer_rng(i))
        ...
        did = doc_id(dt, by, used_document_ids)
        reg = reg_year(ct, by, fy, ni, branches, shock_state)
        hc, hd, hl = historical_res(country, di, li, reg)
        bid = primary_branch(reg, hc, hd, hl, branches)
        status, close = lifecycle(reg, bid, branches)
        ...
        name_cohorts.append(name_cohort)

    chunk_df = pd.DataFrame(rows, columns=COLS)

    for column in ['birth_year', 'foundation_year', 'closing_year']:
        chunk_df[column] = chunk_df[column].astype('Int64')

    for column in ['monthly_income', 'annual_revenue']:
        chunk_df[column] = pd.to_numeric(
            chunk_df[column],
            errors='coerce',
        ).astype('Float64')

    chunk_df['primary_branch_id'] = (
        chunk_df['primary_branch_id']
        .astype('string')
        .str.zfill(3)
    )

    return chunk_df, name_cohorts


def generate(chunk_size: int = 5_000):
    """
    Generate customers incrementally into one Parquet file.

    Each generation chunk is written as a Parquet row group. Customer rows are
    not accumulated in memory during generation. The completed Parquet dataset
    is then loaded once for the existing validation and diagnostic report.
    """
    ...
    set_active_rng(make_global_rng())

    shock_state = shocks()
    n = WORLD.customer_count
    share = active_rng().uniform(.08, .15)
    types = active_rng().choice(
        ['INDIVIDUAL', 'BUSINESS'],
        n,
        p=[1-share, share],
    )
    ...
    print(f"RNG_STREAM_GLOBAL: {RNG_STREAM_GLOBAL}")
    print(f"RNG_STREAM_ENTITY_BASE: {RNG_STREAM_ENTITY_BASE}")
    print(f"Execution mode: {WORLD.execution_mode.upper()}")
    ...
    try:
        for chunk_start in range(0, n, chunk_size):
            ...
            chunk_df, chunk_name_cohorts = _build_customer_chunk(
            ...
            parquet_writer.write_table(
                table,
                row_group_size=len(chunk_df),
            )

            rows_written += len(chunk_df)
            audit_name_cohorts.extend(chunk_name_cohorts)

            print(
                f"  ... {rows_written:,}/{n:,}"
                f" | row group rows: {len(chunk_df):,}"
            )
    ...
    if rows_written != n:
        raise RuntimeError(
            f"Parquet generation incomplete: wrote {rows_written:,} "
            f"of {n:,} customers."
        )

    df = pd.read_parquet(PARQUET_OUT)

    # Internal audit-only field. It is never written to customers.parquet.
    df['_name_cohort'] = pd.Series(
        audit_name_cohorts,
        dtype='string',
    )
...
def main():
    ...
    parquet_size_mb = PARQUET_OUT.stat().st_size / (1024 ** 2)
    ...
    if WRITE_COMPATIBILITY_CSV:
        csv_size_mb = CSV_OUT.stat().st_size / (1024 ** 2)
        print(
            f'Compatibility CSV: {CSV_OUT}'
