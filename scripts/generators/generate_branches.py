...
Dynamic branch-network generator

Creates:
    data/generated/core/branches.csv
    data/interim/world/branch_yearly_state.csv
...
OUTPUT_PATH = GENERATED_CORE_DIR / "branches.csv"
MACRO_PATH = GENERATED_WORLD_DIR / "macro_environment.csv"
STATE_PATH = INTERIM_WORLD_DIR / "branch_yearly_state.csv"
...
BRANCH_WORLD_SEED = WORLD.seed
RNG_NAMESPACE = "branches"
...
def load_macro_environment() -> pd.DataFrame:
    """Load the canonical world-level macro environment."""
    if not MACRO_PATH.exists():
        raise FileNotFoundError(
            "Missing canonical macro environment. Run generate_macro_environment.py first:\n"
            f"  {MACRO_PATH}"
        )
    ...
    if missing:
        raise ValueError(
            "macro_environment.csv is missing required columns: "
            + ", ".join(sorted(missing))
        )
    ...
    if set(macro["year"].astype(int)) != set(YEARS):
        raise ValueError(
            f"Macro environment must cover {OBSERVATION_START_YEAR}-{CURRENT_YEAR}."
        )
    ...
    macro_seeds = (
        pd.to_numeric(macro["world_seed"], errors="raise")
        .astype(int)
        .unique()
    )
    if len(macro_seeds) != 1:
        raise ValueError(
            "Macro environment must contain exactly one world_seed."
        )
    if int(macro_seeds[0]) != int(WORLD.seed):
        raise ValueError(
            "Macro environment world_seed does not match the canonical "
            "BTYT world seed."
        )
...
def _simulate_branch_world(
    ...
    """
    rng_state = make_rng(world_seed, RNG_NAMESPACE, RNG_STREAM_STATE)
    rng_shock = make_rng(world_seed, RNG_NAMESPACE, RNG_STREAM_LOCAL_SHOCK)
    rng_close = make_rng(world_seed, RNG_NAMESPACE, RNG_STREAM_CLOSURE_DRAW)
    rng_reason = make_rng(world_seed, RNG_NAMESPACE, RNG_STREAM_REASON)
...
def print_report(
    ...
    print("\n" + "=" * 96)
    print("BTYT BRANCH NETWORK GENERATOR")
    print("=" * 96)
...
def main() -> None:
    ...
    branches, state_df = simulate_branch_closures(
        structural=structural,
        macro=macro,
        world_seed=BRANCH_WORLD_SEED,
    )

    validate_branches(branches)
    validate_state(branches, state_df)
    print_report(branches, state_df, BRANCH_WORLD_SEED, macro)

    GENERATED_CORE_DIR.mkdir(parents=True, exist_ok=True)
