...
from scripts.core.paths import GENERATED_CORE_DIR, INTERIM_WORLD_DIR
...
# - Canonical customer population from world_config.json.
# - Pre-observation history is compressed inherited state.
# - The configured observation window is modeled as explicit annual issuance / closure evolution.
...
RNG_NAMESPACE = "cards"
RNG_STREAM_LATENT_BASE = 300_000
RNG_STREAM_HISTORICAL_DEBIT_BASE = 400_000
RNG_STREAM_HISTORICAL_CREDIT_BASE = 500_000
RNG_STREAM_OBSERVED_BASE = 600_000
...
PARQUET_OUTPUT_PATH = GENERATED_CORE_DIR / "cards.parquet"
CSV_OUTPUT_PATH = GENERATED_CORE_DIR / "cards.csv"
...
def _read_parquet_or_csv(parquet_path, csv_path, dtype=None):
    if parquet_path.exists():
        frame = pd.read_parquet(parquet_path)
        source = parquet_path
    elif csv_path.exists():
        frame = pd.read_csv(csv_path, dtype=dtype)
        source = csv_path
    else:
        raise FileNotFoundError(
            f"Missing canonical BTYT input. Expected either {parquet_path} or {csv_path}."
        )

    if dtype:
        for column, target in dtype.items():
            if column in frame.columns and target is str:
...
def load_inputs():
    if not PRODUCTS_PATH.exists():
        raise FileNotFoundError(f"Missing canonical BTYT input: {PRODUCTS_PATH}")
    if not BRANCH_STATE_PATH.exists():
        raise FileNotFoundError(f"Missing canonical BTYT input: {BRANCH_STATE_PATH}")

    customers, customer_source = _read_parquet_or_csv(
        CUSTOMERS_PARQUET_PATH,
        CUSTOMERS_CSV_PATH,
        dtype={"customer_id": str, "primary_branch_id": str},
    )
    accounts, account_source = _read_parquet_or_csv(
...
def validate_inputs(customers, accounts, products, branch_state):
    ...
    if unknown_accounts:
        raise ValueError(
            "accounts contains customer IDs outside the canonical "
            f"{len(customers):,}-customer world. "
...
class CardsGenerator:
    def __init__(self, customers, accounts, branch_state):
        ...
        # The active RNG is assigned from deterministic entity-specific streams.
        # This prevents customer chunk boundaries from changing the realized world.
        self.rng = make_rng(WORLD.seed, RNG_NAMESPACE, 0)
    ...
    @staticmethod
    def customer_number(customer_id):
        digits = "".join(ch for ch in str(customer_id) if ch.isdigit())
        if not digits:
            raise ValueError(f"Cannot derive numeric stream from customer_id={customer_id!r}")
        return int(digits)
    ...
    def account_number(account_id):
        ...
        if not digits:
            raise ValueError(f"Cannot derive numeric stream from account_id={account_id!r}")
        return int(digits)

    def set_rng(self, stream):
        self.rng = make_rng(WORLD.seed, RNG_NAMESPACE, int(stream))
    ...
    def prepare_branch_state(self):
        """
        Build the configured observation-period branch-state lookup used by the card DGP.
    ...
    def generate_card_side_latents(self):
        ...
        latent_draws = np.empty((n, 5), dtype=float)
        for idx, customer_id in enumerate(c["customer_id"]):
            rng = make_rng(
                WORLD.seed,
                RNG_NAMESPACE,
                RNG_STREAM_LATENT_BASE + self.customer_number(customer_id),
            )
            latent_draws[idx, :] = rng.normal(0, 1, 5)

        common = latent_draws[:, 0]
        noise_depth = latent_draws[:, 1]
        noise_digital = latent_draws[:, 2]
        noise_usd = latent_draws[:, 3]
        noise_credit = latent_draws[:, 4]
    ...
    def choose_issue_channel(self, row, year, product, branch_id=None):
        ...
        elif year < OBSERVATION_START_YEAR:
            scores = {"BRANCH": 1.20, "REMOTE_ASSISTED": 0.30, "DIGITAL": 0.30}
        else:
            progress = np.clip(
                (year - OBSERVATION_START_YEAR)
                / max(1, CURRENT_YEAR - OBSERVATION_START_YEAR),
                0.0,
                1.0,
            )
            scores = {
    ...
    def debit_survival_probability_to_2021(self, row, account, issue_year):
        age = OBSERVATION_START_YEAR - issue_year
        redundancy = len(
            [
                c for c in self.portfolios.get(str(row.name), [])
                if c["product_id"] == "P009" and c["issue_year"] < OBSERVATION_START_YEAR
            ]
        )
    ...
    def generate_pre_2021_debit(self):
        for account in self.accounts.itertuples(index=False):
            self.set_rng(
                RNG_STREAM_HISTORICAL_DEBIT_BASE + self.account_number(account.account_id)
            )
            if account.product_id not in DEBIT_ACCOUNT_PRODUCTS:
                continue
            if int(account.opening_year) >= OBSERVATION_START_YEAR:
                continue
            ...
            if account_close is not None and account_close < OBSERVATION_START_YEAR:
                forced_closed = True
            if (
                str(row["customer_status"]) == "CLOSED"
                and customer_close is not None
                and customer_close < OBSERVATION_START_YEAR
            ):
                forced_closed = True
    ...
    def historical_credit_count(self, row):
        ...
        # Compressed pre-observation "ever acquired credit" probability.
        score = (
            -0.65
    ...
    def generate_pre_2021_credit(self):
        for customer_id, row in self.customer_lookup.iterrows():
            self.set_rng(
                RNG_STREAM_HISTORICAL_CREDIT_BASE + self.customer_number(customer_id)
            )
            if str(row["customer_type"]) != "INDIVIDUAL":
                continue
            if int(row["registration_year"]) >= OBSERVATION_START_YEAR:
                continue
    ...
    # -------------------------------------------------------------------------
    # Observed-period debit issuance
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # Observed-period credit issuance
    # -------------------------------------------------------------------------
    ...
    def generate_observed_period(self, customer_ids=None):
        if customer_ids is None:
            customer_ids = list(self.customer_lookup.index)

        for year in range(OBSERVATION_START_YEAR, CURRENT_YEAR + 1):
            year_offset = year - OBSERVATION_START_YEAR
            for customer_id in customer_ids:
                row = self.customer_lookup.loc[customer_id]
                self.set_rng(
                    RNG_STREAM_OBSERVED_BASE
                    + year_offset * 100_000
                    + self.customer_number(customer_id)
                )
...
def audit(cards, customers, accounts, generator):
    print("\n" + "=" * 84)
    print("BTYT CARDS — CANONICAL WORLD AUDIT")
    print("=" * 84)

    print(f"Customers in canonical world:    {len(customers):,}")
    print(f"Canonical accounts:                  {len(accounts):,}")
    ...
    observed_closures = cards[
        pd.to_numeric(cards["closing_year"], errors="coerce").between(OBSERVATION_START_YEAR, CURRENT_YEAR)
    ]
    ...
    inherited = cards[
        (cards["issue_year"] < OBSERVATION_START_YEAR)
        & (
            cards["closing_year"].isna()
            | (pd.to_numeric(cards["closing_year"], errors="coerce") >= OBSERVATION_START_YEAR)
        )
    ]
    print(f"\nInherited card relationships entering {OBSERVATION_START_YEAR}: {len(inherited):,}")
...
def main():
    print("=" * 84)
    print("BTYT CARDS GENERATOR")
    print("=" * 84)
    print(f"World: {WORLD.name}")
    print(f"World seed: {WORLD.seed}")
    print(f"RNG namespace: {RNG_NAMESPACE}")
    print(f"Observation period: {OBSERVATION_START_YEAR}-{CURRENT_YEAR}")
    ...
    if len(customers) != WORLD.customer_count:
        raise ValueError(
            f"Canonical customer count mismatch: expected {WORLD.customer_count:,}, "
    ...
    print(f"\nGenerating compressed pre-{OBSERVATION_START_YEAR} debit-card state...")
    generator.generate_pre_2021_debit()

    print(f"Generating compressed pre-{OBSERVATION_START_YEAR} credit-card state...")
    generator.generate_pre_2021_credit()

    print(
        f"Generating detailed {OBSERVATION_START_YEAR}-{CURRENT_YEAR} "
        "card evolution by customer chunk..."
    )
    customer_ids = generator.customers["customer_id"].tolist()
    total_chunks = math.ceil(len(customer_ids) / CUSTOMER_CHUNK_SIZE)
    for chunk_index, start_idx in enumerate(
        range(0, len(customer_ids), CUSTOMER_CHUNK_SIZE),
        start=1,
    ):
        chunk_ids = customer_ids[start_idx:start_idx + CUSTOMER_CHUNK_SIZE]
        before = len(generator.cards)
        generator.generate_observed_period(chunk_ids)
        added = len(generator.cards) - before
        print(
            f"Processed chunk {chunk_index}/{total_chunks} | "
            f"customers {start_idx + 1:,}-{start_idx + len(chunk_ids):,} | "
            f"new cards {added:,} | cumulative {len(generator.cards):,}"
        )
    ...
    table = pa.Table.from_pandas(cards, preserve_index=False)
    pq.write_table(
        table,
        PARQUET_OUTPUT_PATH,
    ...
    if WRITE_COMPATIBILITY_CSV:
        cards.to_csv(CSV_OUTPUT_PATH, index=False)
    ...
    parquet_mb = PARQUET_OUTPUT_PATH.stat().st_size / (1024 ** 2)
    print(f"\nSaved canonical Parquet: {PARQUET_OUTPUT_PATH}")
    print(f"Parquet size: {parquet_mb:.2f} MB")

    if WRITE_COMPATIBILITY_CSV:
        csv_mb = CSV_OUTPUT_PATH.stat().st_size / (1024 ** 2)
